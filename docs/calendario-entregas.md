...
- [x] **SMS inbound reconcile** (2026-07-08) — resuelto con `sms:reconcile-received` (ver arriba). ⚠️ **Verificar en Swagger del gateway** (`gw.prestamaz.site`) que la ruta de export es `POST /messages/inbox/export`; si difiere, es ajuste de una línea en `SmsGatewayClient::requestInboxExport()`.

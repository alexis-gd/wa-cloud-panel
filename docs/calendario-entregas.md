...
- [ ] **(A) Manejo de números SMS diferenciado — auto-blacklist configurable** *(aprobado, pendiente)*: nuevo `Setting sms_auto_blacklist_bounces` (**default 0 = nunca bloquea**). `Contact::registerSmsBounce()` sigue contando rebotes (para reporte) pero solo pone `sms_blocked` si el umbral > 0 y se alcanza. Razón: SIM propia barata → al cliente no le importa la reputación del número; "no suma fallas". **Opt-out (STOP) se queda igual (legal, LFPDPPP vigente 21-mar-2025)**. WhatsApp mantiene su bloqueo estricto.
- [ ] **(B) SMS entrantes visibles — "Respuestas SMS" (lista plana, NO conversación)** *(aprobado, pendiente)*: guardar los `sms:received` y mostrarlos como inbox plano (fecha · número · mensaje · acción, ej. "Opt-out automático"). Así lo hace la industria de SMS masivo; el hilo tipo chat es para soporte, no para blast. Reusar `conversations` con `channel=sms` o tabla mínima.
- [ ] **(C) Badge "SMS baja" en Contactos** *(aprobado, pendiente)*: chip rojo por canal en la fila cuando `sms_opt_out`/`sms_blocked`/`sms_invalid`, separado del "Estado" (que es WhatsApp). Deja claro "le llega WA pero NO SMS". Tooltip con fecha/origen. Opcional filtro "Baja SMS". El dato `sms_opt_out` ya viaja al frontend; es trabajo de UI + que la Entregabilidad considere el canal SMS.

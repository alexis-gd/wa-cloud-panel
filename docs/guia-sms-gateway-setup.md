...
- **Entrantes** (`sms:reconcile-received`, cada hora): los recibidos viven en el teléfono y NO se
  ...
  `POST {url}/messages/inbox/export` (ruta **confirmada en prod 2026-07-09**). Vuelven por el mismo
  webhook y se deduplican por `gateway_message_id`. Opcional: `SMS_GATEWAY_DEVICE_ID` en `.env` para
  pedir la re-exportación a un device concreto (vacío = todos, recomendado con pool).
